...
def learning_rate_with_decay(
  ...
  """
  initial_learning_rate = flags.FLAGS.learning_rate * batch_size / batch_denom
...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  loss = cross_entropy # Xinyi modified
  reg_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
  # print('Xinyi reg_loss {}'.format(reg_losses))
  if reg_losses:
    loss += tf.add_n(reg_losses) # Xinyi modified
  ...
  # class RestoreHook(tf.train.SessionRunHook): # Xinyi add
    ...
    # def after_create_session(self, session, coord=None):
      # self.restore_model(session, self.ckpt_dir_or_file)
      
    # def restore_model(self, sess, dir_path):
      # print('Xinyi in restore')
      # ckpt = tf.train.get_checkpoint_state(dir_path)
      # ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
      
      # vl = tf.trainable_variables()
      # vl.extend([v for v in tf.global_variables() if "global_step" in v.name])
      # saver = tf.train.Saver(var_list=vl)
      # saver.restore(sess, os.path.join(dir_path, ckpt_name))
  ...
  return tf.estimator.EstimatorSpec(
      ...
      loss=loss,
      train_op=train_op,
      # training_hooks=[RestoreHook(flags.FLAGS.model_dir)], # Xinyi add
...
def solver_func(learning_rate): # Xinyi add
    ...
    opt_name = flags.FLAGS.optimizer
